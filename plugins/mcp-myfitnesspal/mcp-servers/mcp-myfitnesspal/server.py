...
mcp = FastMCP(
    ...
    instructions="""
        ...
        Supports profile-based configuration and secret references.
...
def get_client() -> MyFitnessPalClient:
    ...
    if _client is None:
        ...
        _client = MyFitnessPalClient(cookies_json=runtime_config.mfp_cookies)
...
def format_tool_error(prefix: str, error: Exception) -> str:
    """Return safe, actionable tool errors without exposing secret values."""
    if isinstance(error, ConfigError):
        return f"{prefix}: configuration setup needed ({error})"
    return f"{prefix}: {error}"


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_daily_summary(date: Optional[str] = None) -> ToolResult:
    """
...
"""
        ...
        return text_response(format_tool_error("Error retrieving daily summary", e))


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_daily_meals(date: Optional[str] = None) -> ToolResult:
    """
    ...
    try:
        ...
        return text_response(format_tool_error("Error retrieving meals", e))


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_daily_exercise(date: Optional[str] = None) -> ToolResult:
    """
    ...
    try:
        ...
        return text_response(format_tool_error("Error retrieving exercise", e))


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_daily_macros(date: Optional[str] = None) -> ToolResult:
    """
    ...
    try:
        ...
        return text_response(format_tool_error("Error retrieving macros", e))


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_water_intake(date: Optional[str] = None) -> ToolResult:
    """
    ...
    try:
        ...
        return text_response(format_tool_error("Error retrieving water intake", e))


@mcp.tool(annotations=TOOL_ANNOTATIONS)
def get_date_range_summary(start_date: str, end_date: str) -> ToolResult:
    """
    ...
    try:
        ...
        return text_response(format_tool_error("Error retrieving date range summary", e))
