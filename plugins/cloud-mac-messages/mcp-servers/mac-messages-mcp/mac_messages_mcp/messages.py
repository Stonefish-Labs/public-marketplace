...
def get_chats() -> str:
    ...
    try:
        ...
        if not results: return "No group chats found."
        if "error" in results[0]: return f"Error accessing chats: {results[0]['error']}"
        chats = [r for r in results if r.get('display_name')]
        if not chats: return "No named group chats found."
        formatted_chats = []
...
def check_contacts() -> str:
    ...
    try:
        contacts = get_cached_contacts()
        if not contacts: return "No contacts found in AddressBook."
        contact_count = len(contacts)
        sample_entries = list(contacts.items())[:10]
...
def format_find_contact(name: str) -> str:
    """Find a contact by name and return a formatted string result."""
    try:
        matches = find_contact_by_name(name)
        if not matches: return f"No contacts found matching '{name}'."
        if len(matches) == 1:
            c = matches[0]
            return f"Found contact: {c['name']} ({c['phone']}) with confidence {c['score']:.2f}"
        result = [f"Found {len(matches)} contacts matching '{name}':"]
        for i, c in enumerate(matches[:10]):
            result.append(f"{i+1}. {c['name']} ({c['phone']}) - confidence {c['score']:.2f}")
        if len(matches) > 10: result.append(f"...and {len(matches) - 10} more.")
        return "\n".join(result)
    except Exception as e:
        return f"Error finding contact: {str(e)}"

def check_imessage_availability(recipient: str) -> str:
